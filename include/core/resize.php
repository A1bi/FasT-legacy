...
class resize {
	...
	function resizepic($pic,$dir,$size) {
		...
		$file=$_base."gfx/cache/".$dir."/full/".$pic;
	...
	function del_pic($dir,$size,$id) {
		global $_base;
		$file=$_base."gfx/".$dir."/".$size."/".$id.".jpg";
		if (file_exists($file)) {
			@unlink($file);
		}
	}
	function gallery($id,$aid) {
		$this->resizepic($id,"gallery/".$aid,"small");
		$this->resizepic($id,"gallery/".$aid,"medium");
		return true;
	}
	function slide($id) {
		$this->resizepic($id,"slides/","slide");
		return true;
	}
}
?>
