{include file="head.tpl" cssfile="index"}
		<div class="hl">
			Herzlich willkommen!
		</div>
		<div class="welcome hcen">
			Die Freilichtbühne am schiefen Turm heißt Sie herzlich willkommen auf unserer Homepage!
		</div>
		<div class="trenner"></div>
		<div class="teaser">
			<div class="hl">
				Sommernachtstheater 2012
				{if $smarty.now > 1338760800}<div>Der Vorverkauf hat begonnen!</div>{/if}
			</div>
			<div class="title hcen">
				„Das Haus in Montevideo“ auf der Escher Freilichtbühne<br />Sommernachtstheater 2012 für die ganze Familie
			</div>
			<div class="left"><img src="/gfx/theater/montevideo/title.png" alt="" /></div>
			<div class="right">
				<a href="/theater/montevideo" class="hl">Das Stück</a>
				<a href="/termine" class="hl">Termine</a>
				<a href="/tickets" class="hl">Karten bestellen</a>
			</div>
		</div>
{include file="foot.tpl"}